@extends('deskapp.app')
@section('tittle', 'Nomor LDP Tim KEKDA')
@section('judul', 'Nomor LDP Tim KEKDA')
@section('halaman', 'Nomor LDP Tim KEKDA')
@section('submenu', 'Nomor Ambil LDP Tim KEKDA')
@section('container')
    <div class="pd-20 card-box mb-30">
      <div class="clearfix">
        <div class="pull-left">
          <h3 class="text-blue h4">Form Ambil Nomor LDP Tim KEKDA</h3>
        </div>
      </div>
        <div class="dropdown-divider"></div>
        <br>
        <form method="POST" id="quickForm" action="{{ route('kekdaldp.simpan') }}">
          @csrf  
            ...
            <div class="form-group">
              ...
              <input type="text" name="nominal" class="form-control @error('nominal') is-invalid @enderror" value="{{ old('nominal') }}" id="nominal" placeholder="Masukkan nominal pembayaran (format : 100000 tanpa titik dan koma), kosongkan jika tidak ada">
            </div>
           @error('nominal')
              <div class="alert alert-danger" role="alert">{{ $message }}</div>
            @enderror
