@extends('deskapp.app')
@section('tittle', 'FORM TV 01 UIPUR')
@section('judul', 'FORM TV 01 UIPUR')
@section('halaman', 'FORM TV 01 UIPUR')
@section('container')
@if (session('status'))
...
<div class="card-box mb-30">
  <div class="pd-20">
    <h4 class="text-blue h4"><a href="{{ route('uipurtv01.ambil') }}"><button class="btn btn-primary"><i class="fa fa-plus"></i> | Ambil Nomor</button></a></h4>
  </div>
    <div class="pb-20">
          ...
          <td>
            <div class="btn-group">
              ...
              <div class="dropdown-menu" role="menu">
                ...
                <a class="dropdown-item" href="./uipurtv01/ubah/{{$show->id}}"><i class="dw dw-edit2"></i> | Edit</a>
                @endif
                @if ((Auth::user()->level == 'admin') or (Auth::user()->name == $show->user) && ( \Carbon\Carbon::now()->isoFormat('D MMMM Y') == \Carbon\Carbon::parse($show->created_at)->isoFormat('D MMMM Y')))
                <form action="./uipurtv01/hapus/{{$show->id}}"    method="POST">
                  {{ method_field('delete') }}
                  {{ csrf_field() }}
