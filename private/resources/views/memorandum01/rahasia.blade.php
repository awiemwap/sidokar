@extends('deskapp.app')
@section('tittle', 'Memorandum M01 Rahasia')
...
<div class="card-box mb-30">
  <div class="pd-20">
    <h4 class="text-blue h4"><a href="{{ route('m01rahasia.ambil') }}"><button class="btn btn-primary"><i class="fa fa-plus"></i> | Ambil Nomor</button></a></h4>
  </div>
    <div class="pb-20">
          ...
          <td>
            <div class="btn-group">
              ...
              <div class="dropdown-menu" role="menu">
                ...
                <a class="dropdown-item" href="./m01rahasia/ubah/{{$show->id}}"><i class="dw dw-edit2"></i> | Edit</a>
                @endif
                @if ((Auth::user()->level == 'admin') or (Auth::user()->name == $show->user) && ( \Carbon\Carbon::now()->isoFormat('D MMMM Y') == \Carbon\Carbon::parse($show->created_at)->isoFormat('D MMMM Y')))
                <form action="./m01rahasia/hapus/{{$show->id}}"    method="POST">
                  {{ method_field('delete') }}
                  {{ csrf_field() }}
                ...
                @endif
                <a class="dropdown-item" href="./m01rahasia/backdate/{{$show->id}}"><font color="red"><i class="bi bi-arrow-left-square"></i> | Nomor Backdate</font></a>
                @if (Auth::user()->level == 'admin')
                <a class="dropdown-item" href="./m01rahasia/edit/{{$show->id}}"><font color="red"><i class="icon-copy fa fa-edit" aria-hidden="true"></i> | Edit Admin</font></a>
                @endif
              </div>
            </div>
          </td>
