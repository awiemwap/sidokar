@extends('deskapp.app')
@section('tittle', 'FORM TV 01 UIKSPPSPPUR')
@section('judul', 'FORM TV 01 UIKSPPSPPUR')
@section('halaman', 'FORM TV 01 UIKSPPSPPUR')
@section('container')
@if (session('status'))
...
<div class="card-box mb-30">
  <div class="pd-20">
    <h4 class="text-blue h4"><a href="{{ route('uiksptv01.ambil') }}"><button class="btn btn-primary"><i class="fa fa-plus"></i> | Ambil Nomor</button></a></h4>
  </div>
    <div class="pb-20">
      <table id="example1" class="table hover multiple-select-row data-table-export nowrap">
        ...
        <tr>
          ...
          <td class="table-plus">{{ $show->pelaksana }}</td>
          <td class="table-plus">{{ $show->kegiatan }}</td>
          <td class="table-plus">{{ $show->kota }}</td>
          ...
          <td>
            <div class="btn-group">
              ...
              <div class="dropdown-menu" role="menu">
                ...
                <a class="dropdown-item" href="./uiksptv01/ubah/{{$show->id}}"><i class="dw dw-edit2"></i> | Edit</a>
                @endif
                @if ((Auth::user()->level == 'admin') or (Auth::user()->name == $show->user) && ( \Carbon\Carbon::now()->isoFormat('D MMMM Y') == \Carbon\Carbon::parse($show->created_at)->isoFormat('D MMMM Y')))
                <form action="./uiksptv01/hapus/{{$show->id}}"    method="POST">
                  {{ method_field('delete') }}
                  {{ csrf_field() }}
                  ...
                  <a class="dropdown-item" href="./uiksptv01/edit/{{$show->id}}"><font color="red"><i class="icon-copy fa fa-edit" aria-hidden="true"></i> | Edit Admin</font></a>
                  @endif
                </form>
                @endif
                {{-- <a class="dropdown-item" href="/uipurtv01/backdate/{{$show->id}}"><font color="red"><i class="bi bi-arrow-left-square"></i> | Nomor Backdate</font></a>
                @if (Auth::user()->level == 'admin')
                <a class="dropdown-item" href="/uipurtv01/edit/{{$show->id}}"><font color="red"><i class="icon-copy fa fa-edit" aria-hidden="true"></i> | Edit Admin</font></a>
                @endif --}}
              </div>
            </div>
          </td>
